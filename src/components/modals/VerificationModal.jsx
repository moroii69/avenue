    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (userID && authToken) {
          ...
          // Use either the user object or build from available data
          if (user) {
            ...
            // Identify user in OpenPanel analytics
            try {
              identifyUser(userID, {
            ...
            // Basic identification if we only have the phone number and ID
            try {
              identifyUser(userID, {
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-xs">
        <div className="bg-[#151515] rounded-xl p-8 shadow-xl relative flex flex-col items-center">
          ...
          <button
            ...
            className="text-gray-200 text-sm underline mt-5 font-inter"
          >
            Resend code
          </button>
        </div>
