...
const Home = () => {
  ...
  const clients = [
    {
      ...
      image:
        "https://banner2.cleanpng.com/20180821/bo/cce051a88d8bd9b01c4f77d10c29693b.webp",
      location: "New York",
      price: "50",
    },
    {
      ...
      image:
        "https://img.freepik.com/premium-photo/colorful-confetti-falling-crowd-party-photo_1233553-2491.jpg?semt=ais_hybrid",
    ...
    },
    {
      ...
      image:
        "https://hips.hearstapps.com/hmg-prod/images/family-toasting-on-christmas-dinner-at-home-royalty-free-image-1699896250.jpg?crop=1xw:0.84236xh;center,top&resize=1200:*",
      location: "Chicago",
      price: "37",
    },
    {
      ...
      date: "21 Jan 2025 18:00",
      image:
        "https://plus.unsplash.com/premium_photo-1683129651802-1c7ba429a137?fm=jpg&q=60&w=3000&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8cGFydHl8ZW58MHx8MHx8fDA%3D",
      location: "Los Angeles",
      price: "20",
    },
    {
  ...
  useEffect(() => {
    trackPageView("Home Page", {
      section: "user",
      is_logged_in: !!localStorage.getItem("userID"),
    });
    ...
    try {
      if (savedEventIds.has(eventId)) {
        const response = await axios.delete(
          `${url}/saved/delete-saved/${eventId}`,
          {
            data: { user_id: userId },
          }
        );
      ...
      try {
        const isBookmarked = savedEventIds.has(eventId);
        posthog.capture(isBookmarked ? 'event_unbookmarked' : 'event_bookmarked', {
          event_id: eventId
        });
      } catch (error) {
        console.error("Error tracking bookmark action:", error);
      }
    } catch (error) {
  ...
  const handleSearchSubmit = (e) => {
    ...
    try {
      posthog.capture('search_performed', {
        ...
        date_range: {
          start: startDate ? formatDateFilter(startDate) : null,
          end: endDate ? formatDateFilter(endDate) : null
        }
      });
    } catch (error) {
  ...
  return (
    <div className="font-manrope flex flex-col items-center justify-center text-center mt-14 bg-primary px-4">
      ...
      <h1 className="text-5xl sm:text-5xl md:text-6xl lg:text-[76px] font-medium text-white mb-4 tracking-tighter -mt-4 sm:-mt-6 lg:-mt-8">
        in{" "}
        <span className="text-[#8a8686] underline decoration-[#2e2e2e] decoration-4">
          your
        </span>{" "}
        area
      </h1>
      ...
      <div className="flex flex-wrap justify-center mt-8 gap-1 bg-black px-1 py-1 rounded-[1.25rem] border border-[#fff] border-opacity-10">
        {cards.map((card) => (
          ...
          >
            <div className="">
              <div className="mb-1.5 text-white flex justify-center">
                {card.icon}
              </div>
              <div className="flex items-center justify-between">
                <p className="text-md text-white font-medium font-inter text-base leading-[20px]">
                  {card.text}
                </p>
                {card.isActive && (
        ...
        showFreeOnly ? (
          ...
          >
            <span className="text-lg text-white font-medium font-inter">
              Clear all
            </span>
          </button>
        ) : null}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-16 sm:mt-8 px-4 sm:px-8 md:px-16 lg:px-10 bg-primary mb-10">
        {sortedEvents.map((card) => {
          ...
          return (
            <button
              onClick={() =>
                handleDetail(card._id, card.event_name.replace(/\s+/g, "-"))
              }
              key={card.id}
              ...
              <div className="flex items-center justify-between w-full mb-2 gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center mt-1">
                    <FaLocationDot
                      className="text-neutral-400 mr-1 flex-shrink-0"
                      size={12}
                    />
                ...
                <div className="flex-shrink-0">
                  {card.event_type === "ticket" ? (
                    <p className="text-white font-medium whitespace-nowrap">
                      <span className="text-gray-500 text-2xl font-inter">
                        $
                      </span>
