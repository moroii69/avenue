...
import { trackEventView, trackEvent } from "../../utils/analytics";
...
const Info = () => {
  ...
  // Separate useEffect for tracking with PostHog only
  useEffect(() => {
    // Only track when we have a valid event
    const trackView = async () => {
      if (!event || !event._id) return;
      
      try {
        // Track only in PostHog with proper event properties
        posthog.capture('$pageview', {
          ...
          is_user_facing: true,
          tickets_available: event.tickets?.length || 0,
          event_date: event.start_date,
          event_price_min: Math.min(...event.tickets?.map(t => parseFloat(t.price)) || [0]),
          event_price_max: Math.max(...event.tickets?.map(t => parseFloat(t.price)) || [0])
        });
      } catch (error) {
        console.error("Error tracking view in PostHog:", error);
      }
    };

    trackView();
  }, [event?._id]); // Only re-run when event._id changes
  ...
  const handleIncrement = (index) => {
    setTicketCounts((prevCounts) =>
      prevCounts.map((count, i) => {
        if (i === index) {
          const remainingTicket = remain.find(
            (r) => r.ticket_name === event?.tickets[index]?.ticket_name
          );
          const maxCount = event?.tickets[index]?.max_count;
          const currentCount =
            count === null
              ? event?.tickets[index]?.min_count &&
                !isNaN(Number(event.tickets[index].min_count))
                ? Number(event.tickets[index].min_count) - 1
                : 0
              : count;

          const hasValidMaxCount =
            maxCount !== "undefined" &&
            maxCount !== undefined &&
            maxCount !== null &&
            maxCount !== "" &&
          ...
          if (hasValidMaxCount && remainingTicket) {
            limit = Math.min(
              Number(maxCount),
              remainingTicket.remaining_tickets
            );
  ...
  const handleCheckout = () => {
    ...
    trackEvent('checkout_initiated', {
      ...
      ticket_value: totalPrice
    });

    const selectedRemainingTickets =
      remain.find((r) => r.ticket_name === selectedTicket.ticket_name)
  ...
  const fetchEvent = async () => {
    ...
    try {
      const response = await axios.get(
        `${url}/event/get-event-by-name/${encodedName}`
      );
  ...
  const fetchEvents = async () => {
    if (id) {
      try {
        const response = await axios.get(
          `${url}/event/get-event-by-organizer-id/${organizer}`
        );
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        alert(
          "Sharing is not supported on your device. Please copy the URL manually."
        );
      }
    }
  };
  ...
  const handleTicketSelect = (ticket, index) => {
    // ... existing code ...
    ...
    try {
      posthog.capture('ticket_selected', {
        ...
        ticket_price: parseFloat(ticket.price),
        ticket_index: index
      });
    } catch (error) {
      console.error("Error tracking ticket selection:", error);
    }
  };
          ...
          <div className="min-h-screen bg-primary text-white mt-5">
            <div className="max-w-5xl mx-auto px-7">
              <p className="font-inter text-sm">
                <span className="text-[#898989] text-sm font-inter">
                  Home /
                </span>{" "}
                {eventName}
              </p>
              ...
              <div className="flex-1 border border-gray-300 border-opacity-10 px-4 py-4 rounded-2xl overflow-y-auto">
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex items-center space-x-2 mb-8">
                    <span className="text-red-400 border border-[#292929] px-3 py-2 rounded-full font-inter text-sm">
                ...
                <button
                  onClick={() =>
                    handleDetail(
                      ...
                      event?.organizer_id?.url.replace(/\s+/g, "-")
                    )
                  }
                  className="flex mt-5 items-center justify-between bg-[#141414] shadow-md rounded-2xl p-2 w-full max-w-xl"
                >
                  <div className="flex items-center">
                    <div
                      className={`w-12 h-12 rounded-full overflow-hidden mr-4 ${
                        event?.organizer?.profile_image
                          ? "bg-transparent"
                          : "bg-[#121212] border border-[#cccccc]"
                    ...
                    <div>
                      <h2 className="text-md font-bold text-gray-200 font-inter">
                        {event?.organizer_id?.name}
                      </h2>
                      <h2 className="text-xs text-gray-400 font-inter">
                        {
                          events.filter((event) => event.explore === "YES")
                            .length
                        }{" "}
                        Events
                      </h2>
                  ...
                  <>
                    <div className="flex space-x-[-10px] mt-6">
                      {avatars.map((avatar, index) => (
                        <div
                          key={index}
                          className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-semibold border border-black ${
                    ...
                    <div>
                      <p className="mt-1 text-gray-300 w-6/7 text-sm leading-relaxed font-inter">
                        5 others
                      </p>
                    </div>
                  </>
                ...
                <div>
                  <p className="mt-10 text-gray-400 text-xs font-inter font-semibold">
                    ABOUT
                  </p>
                </div>
                        ...
                        <div
                          ...
                          style={{
                            backgroundImage:
                              ...
                              avatar.profile_photo !== "undefined"
                ...
                <div>
                  <p
                    className="mt-4 text-gray-300 w-6/7 text-sm leading-relaxed font-inter"
                ...
                {event.refund_policy ? (
                  <>
                    <div>
                      <p className="mt-6 text-gray-400 text-xs font-inter font-semibold">
                        REFUND POLICY
                      </p>
                    </div>
                    <div>
                ...
                <div>
                  <a
                    href={
                      event.map_link && event.map_link !== "undefined"
                        ? event.map_link
                        : ""
                    }
                    target={
              ...
              <div id="tickets" className="lg:w-96 w-full mb-14">
                {ticketCounts.some((count) => count > 0) && (
                  <div className="bg-[#141414] rounded-2xl p-4 mb-4">
                    <div className="mb-0">
                      <h3 className="text-gray-400 text-xs mb-1 font-inter">
                        TOTAL AMOUNT
                      </h3>
                      <div className="flex justify-between items-center">
                        {event.event_type === "rsvp" ? (
                          ...
                          <>
                            <div className="text-2xl font-medium font-inter">
                              $
                              {ticketCounts
                                .reduce((total, count, index) => {
                                  return (
                                    total +
                                    (count > 0
                                      ? count * event.tickets[index].price
                                      : 0)
                                  );
                                }, 0)
                                .toFixed(2)}
                        ...
                        <div className="flex items-center justify-center">
                          <button
                            ...
                            className="bg-white font-inter text-black py-3 px-6 rounded-full hover:bg-gray-100 text-sm"
                          >
                            Go to checkout
                          </button>
                        </div>
                ...
                {event?.tickets?.map((ticket, index) => {
                  ...
                  const remainingTicket = remain.find(
                    (r) => r.ticket_name === ticket.ticket_name
                  );
                  const isSoldOut =
